...
def get_component_data(api_endpoint: str, api_key: str, component_id: str,
                      limit: int = 20, next_token: str = None) -> dict:
    """
    Query data entries for a component with pagination
    """
    
    query = """
    query GetComponentData($componentId: ID!, $limit: Int, $nextToken: String) {
        listData(componentId: $componentId, limit: $limit, nextToken: $nextToken) {
            items {
                id
                component_id
                workspace_id
