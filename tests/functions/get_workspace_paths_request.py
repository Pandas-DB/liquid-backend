"""Run example:
python3 scripts/queries/get_workspace_paths_request.py ws-20241212083321 --aws-profile test --stage dev --limit 10
...
curl -X POST \
  ...
  -d '{
    "query": "query GetWorkspacePaths($workspaceId: ID!, $limit: Int, $nextToken: String) { listPaths(workspaceId: $workspaceId, limit: $limit, nextToken: $nextToken) { items { id name normalized_name workspace_id created_at updated_at metadata } nextToken } }",
    "variables": {
      "workspaceId": "ws-20241212083321",
      "limit": 10,
      "nextToken": null
    }
  }'
...
def get_workspace_paths(api_endpoint: str, api_key: str, workspace_id: str,
                        limit: int = 20, next_token: str = None) -> dict:
    """
    Query paths in a workspace with pagination
    ...
    query = """
    query GetWorkspacePaths($workspaceId: ID!, $limit: Int, $nextToken: String) {
        listPaths(workspaceId: $workspaceId, limit: $limit, nextToken: $nextToken) {
            items {
                id
                name
                normalized_name
